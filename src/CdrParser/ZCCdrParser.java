...
public class ZCCdrParser extends CdrParser{
	 ...
	 * user_service_CDR_201510211300_00132_20151021#20151021142428#.dat
	 * mds_dns_CDR_201510211300_00006_20151021#20151021142206#.dat
		wap_http_CDR_201510211310_00012_20151021#20151021143212#.dat
		wap_conn_CDR_201510210920_00001_20151021#20151021104201#.ack
		mms_mt_CDR_201510211310_00001_20151021#20151021143201#.dat
		mms_mo_CDR_201510211310_00001_20151021#20151021143201#.dat
		gn_CDR_201510211310_00001_20151021#20151021143201#.dat
	...
	public int parse() {
		...
		int pos1=this.cName.indexOf("_CDR");
		this.cPrefix=this.cName.substring(0, pos1);
		
		int pos=this.cPrefix.length()+"_CDR_".length();		
		this.cDate=this.cName.substring(pos, pos+8);
		
		//按天入库
		
		this.cTargetTable=CConf.getCdrHTableName(cFirm,cPrefix)+"-"+this.cDate;
		//System.out.println(this.cTargetTable);
		...
		//this.ts2idx=CConf.getHWCdrTimeStampIdx(this.cPrefix, 2);
		...
		//System.out.println("============>"+cColumnCount);
		this.cDurablity=CConf.getHtableDurability(cFirm, cPrefix);
		
		}catch(Exception e)
		{
			CLogger.log4j("ERROR","ZCCdrParser.java, parse error,"+e.toString());
	 ...
	 */
	public byte[] generatetRowKey(final String[] r){
		// TODO Auto-generated method stub
		//[4][8][8][4]
		//String r[]=rowItems;
		...
		//使用starttime作为cdr到检索时间
		long ts=StrUtils.datestr2long2(r[this.ts1idx]);
		...
		return Bytes.add(Bytes.add(Bytes.toBytes(salt), 
                        ...
                        Bytes.toBytes(ts)),Bytes.toBytes(rnum));
	
		}catch(Exception e){
			//e.printStackTrace();
			CLogger.log4j("ERROR","ZCCdrParser, generatetRowKey Exception, "+e.toString());
			//CLogger.logStackTrace(e);
			return null;
		}
	...
	public ArrayList<byte[]> generateIndexRowkey(String[] r, int rNum, byte[] rk)
	{
		...
		for(IndexObj obj:csip.getParseIdxObj())
		{
			...
			CLogger.log4j("ERROR","ZCCdrParser, generateIndexRowkey Exception, "+e.toString()+"\n");
			CLogger.logStackTrace(e);
		}
		finally{
			return rkAL;
